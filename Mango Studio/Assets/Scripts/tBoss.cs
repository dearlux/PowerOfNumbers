...
public class tBoss : MonoBehaviour {
	...
	//Boss1 things
	public tBossModel model1;
	public float speed;
	public tGameController m;
	...
	public void init (tGameController owner, int type) {
		bossType = type;
		this.name = "Boss";
		m = owner;
		...
		if (bossType == 1) {
			...
			model1 = modelObject.AddComponent<tBossModel> ();						// Add a marbleModel script to control visuals of the gem.
			model1.init (this);
			...
			this.bossHealth = 100;
			var modelObject = GameObject.CreatePrimitive(PrimitiveType.Quad);	// Create a quad object for holding the gem texture.
			model2 = modelObject.AddComponent<tBoss2Model>();						// Add a marbleModel script to control visuals of the gem.
			model2.init(this);
			...
			tBossShield shield = shieldObject.AddComponent<tBossShield>();
			shield.init (this);
...
//		}
	...
	void FireTracer(){ 						//I made this take x and y because I was thinking about it and different enemies will need to fire from different parts of their models
		GameObject bulletObject = new GameObject();		
		tTracerBullet bullet = bulletObject.AddComponent<tTracerBullet>();
